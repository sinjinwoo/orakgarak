...
const RecommendationsPage: React.FC = () => {
  ...
  const [currentStep, setCurrentStep] = useState<
    "welcome" | "test" | "recommendations" | "history"
  >("welcome");
  ...
  const currentRecommendation = useMemo(() => {
    return recommendationHistory.find(
      (rec) => rec.id === currentRecommendationId
    );
  ...
  const filteredSongs = useMemo(() => {
    ...
    return recommendedSongs.filter((song) => {
      if (
        song.vocalRange.min < filter.vocalRange.min ||
        song.vocalRange.max > filter.vocalRange.max
      ) {
        return false;
      }
      return true;
  ...
  const generateNewRecommendation = useCallback(
    async (voiceAnalysis: VoiceAnalysis | null) => {
      const recommendationId = `rec_${Date.now()}_${Math.random()
        .toString(36)
        .substr(2, 9)}`;
      ...
      try {
        ...
        if (!voiceAnalysis) {
          ...
          const recommendedSongs =
            await songService.getPersonalizedRecommendations(20);
            ...
            .map((musicData) => {
              const score = calculateRecommendationScore(
                ...
                musicData,
                {
                  ...
                  difficulty:
                    filter.difficulty !== "all" ? filter.difficulty : undefined,
                  mood: filter.mood,
                }
              );
  ...
  // 곡 피드백 핸들러
  const handleSongFeedback = useCallback(
    (songId: string, feedback: "like" | "dislike") => {
      setUserFeedback((prev) => {
        const newFeedback = { ...prev, [songId]: feedback };

        // 통계 업데이트
        const likes = Object.values(newFeedback).filter(
          (f) => f === "like"
        ).length;
        const dislikes = Object.values(newFeedback).filter(
          (f) => f === "dislike"
        ).length;
      ...
      setSnackbar({
        ...
        message:
          feedback === "like"
            ? "좋아요! 비슷한 곡을 더 추천해드릴게요"
            : "피드백 감사합니다! 다른 곡을 추천해드릴게요",
            ...
            >
              ...
              >
                ...
                <Box
                  ...
                  sx={{
                    background:
                      "linear-gradient(135deg, rgba(251, 66, 212, 0.1) 0%, rgba(175, 15, 90, 0.1) 100%)",
                    border: "2px solid rgba(251, 66, 212, 0.3)",
                    borderRadius: "25px",
                    p: 4,
                    cursor: "pointer",
                ...
                >
                  <Box
                    sx={{
                      ...
                      bottom: 0,
                      background:
                        "linear-gradient(135deg, rgba(251, 66, 212, 0.05) 0%, transparent 50%)",
                      zIndex: 1,
                    }}
                  />
              ...
              <Box sx={{ position: "relative", zIndex: 1 }}>
                ...
                >
                  <Box>
                    ...
                    >
                      {currentRecommendation?.voiceAnalysis
                        ? "🎤 맞춤 추천 곡"
                        : "🎵 인기 추천 곡"}
                    </Typography>
                    <Typography
                    ...
                    <Box sx={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
                      ...
                      {recommendationStats.averageScore > 0 && (
                        ...
                        >
                          ...
                          >
                            📊 만족도{" "}
                            {Math.round(recommendationStats.averageScore)}%
                          </Typography>
                        </Box>
                      )}
                    </Box>
            ...
            >
              ...
              <Box sx={{ position: "relative", zIndex: 1 }}>
                ...
                <Typography variant="body1" sx={{ color: "#94a3b8" }}>
                  총 {recommendationHistory.length}개의 추천 세션을 확인할 수
                  있습니다
                </Typography>
              </Box>
            </Box>
