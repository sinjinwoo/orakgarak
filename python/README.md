...
## 사용자 음성 맞춤 추천 기능 
- 사용자 목소리의 특성을 추출하고 어울리는 노래를 추천합니다.

## Feature 추출
사용자의 음성과 곡 데이터에서 음색/톤 관련 특징을 추출하여 추천에 활용합니다.
...
```mermaid
flowchart TD
    A[사용자 음성 파일 입력] --> B[Mel-Spectrogram 추출]
    B --> C[MFCC & Pitch 특징 추출]
    C --> D[User Feature CSV 저장]
    D --> E[전체 곡 Feature CSV 불러오기]
    E --> F[정규화 + 유사도 계산]
    F --> G[Popularity + Pitch 필터링]
    G --> H[상위 N개 추천 곡 출력]
```

좋습니다 👍
README에 **“전체 파이프라인 실행(main.py)”** 외에도,
**단일 기능만 수행하고 싶을 때 실행해야 하는 파일**을 추가로 정리하면 더 깔끔할 것 같네요.

아래처럼 **추가 섹션**을 두는 걸 추천합니다.
...
```mermaid
flowchart TD
    A[main.py 실행] --> B[사용자 음성 파일 입력]
    B --> C[Mel-Spectrogram 추출]
    C --> D[MFCC & Pitch 특징 추출]
    D --> E[user_features.csv 생성]
    E --> F[전체 곡 Feature(all_features.csv) 불러오기]
    F --> G[정규화 + 유사도 계산]
    G --> H[Popularity + Pitch 필터링]
    H --> I[상위 N개 추천 곡 출력]
```
...
```bash
python user/user_extract_feature.py
```
