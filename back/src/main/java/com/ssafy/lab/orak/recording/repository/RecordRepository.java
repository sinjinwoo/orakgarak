...
public interface RecordRepository extends JpaRepository<Record, Long> {
    ...
    @Query("SELECT r FROM Record r JOIN FETCH r.upload u WHERE r.userId = :userId AND u.uploadDirectory = :directory")
    List<Record> findByUserIdAndUploadDirectoryWithUpload(@Param("userId") Long userId, @Param("directory") String directory);

    @Query("SELECT r FROM Record r JOIN FETCH r.upload u WHERE u.uploadDirectory = :directory")
